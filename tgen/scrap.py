...
from tgen.util.comparison_util import SupportedComparisonFunction, ComparisonFunction
...
class LLMClustering(iClustering):
    ...
    def _get_features(artifact_ids: List[str], artifact_content: List[str], target_artifact_type: str,
        ...
        prompt = SupportedPrompts.FUNCTIONALITIES.value.format_value(target_artifact_type=target_artifact_type)
        res = LLMClustering._get_response(artifact_content, llm_manager, prompt, artifact_ids)
    ...
    def _get_clusters(artifact_ids: List[str], artifact_content: List[str], target_artifact_type: str,
        ...
        prompt = base_prompt.format_value(features=features, target_artifact_type=target_artifact_type)
        res = LLMClustering._get_response(artifact_content, llm_manager, prompt, artifact_ids)
    ...
    def _recluster_large_groups(artifact_df: ArtifactDataFrame, clusters: Clusters, target_artifact_type: str,
        ...
        """
        okay_clusters = LLMClustering._filter_clusters_by_size(clusters, LLMClustering.CLUSTER_MAX, SupportedComparisonFunction.MIN)
        okay_features = list(okay_clusters.keys())
        clusters2redo = {cluster_id: artifacts for cluster_id, artifacts in clusters.items() if cluster_id not in okay_clusters}
        logger.info(f"\nRe-clustering {len(clusters2redo)} clusters with more than {LLMClustering.CLUSTER_MAX} artifacts")
        for cluster_name, artifacts in clusters2redo.items():
            artifact_content = [artifact_df.get_artifact(id_)[ArtifactKeys.CONTENT] for id_ in artifacts]
            prompt = SupportedPrompts.RE_CLUSTER_FEATURE.value.format_value(feature=cluster_name)
            new_clusters = LLMClustering._get_clusters(artifacts, artifact_content, target_artifact_type,
                                                       llm_manager, features=okay_features,
                                                       base_prompt=prompt)
            LLMClustering._update_clusters(okay_clusters, new_clusters)
    ...
    def _recluster_small_groups(artifact_df: ArtifactDataFrame, clusters: Clusters, target_artifact_type: str,
        ...
        """
        okay_clusters = LLMClustering._filter_clusters_by_size(clusters, LLMClustering.CLUSTER_MIN, SupportedComparisonFunction.MAX)
    ...
    @staticmethod
    def _get_response(artifact_content: List[str], llm_manager: AbstractLLMManager, prompt: Union[SupportedPrompts, str],
                      artifact_ids: List[str] = None) -> str:
        """
        ...
        contents = [LLMClustering.format_artifact_content(i, artifact_ids[i], content) for i, content in enumerate(artifact_content)] \
            if artifact_ids else artifact_content
        ...
        batch_res = llm_manager.make_completion_request(completion_type=LLMCompletionType.GENERATION, prompt=prompt, **params)
        res = batch_res.batch_responses[0]
        return res
    ...
    def _get_cluster_name_and_artifacts(group: bs4.Tag, artifact_ids: List[str]) -> Tuple[str, List[str]]:
        ...
        """
        name, features = '', []  # defaults
        for child in group.children:
            if isinstance(child, bs4.Tag) and child.contents is not None and len(child.contents) > 0:
                if child.name == LLMClustering.ARTIFACT_ID_TAG:
                    name = LLMClustering._get_artifact_id_by_num(child.contents[0], artifact_ids)
                if child.name == LLMClustering.FEATURES_TAG:
                    features = child.contents[0].split(COMMA)
        return name, features

    @staticmethod
    def _filter_clusters_by_size(clusters: Clusters, threshold: int, comparison_strategy: ComparisonFunction) -> Clusters:
        """
        Filters clusters out that do not meet the threshold
        :param clusters: The clusters to filter
        :param threshold: The min or max value allowed
        :param comparison_strategy: Determines whether a max or min strategy is used
        :return: The filtered clusters
        """
        return {name: artifacts for name, artifacts in clusters.items() if comparison_strategy(len(artifacts), threshold)}

    @staticmethod
    def _update_clusters(original_clusters: Clusters, new_clusters: Clusters) -> Clusters:
        """
        Updates the original clusters with the artifacts from the new clusters
        :param new_clusters: The new clusters to update the original clusters with
        :param original_clusters: The original clusters
        :return: The updated clusters
        """
        for name, artifacts in new_clusters.items():
            if name in original_clusters:
                existing_artifacts = set(original_clusters[name])
                original_clusters[name].extend([art for art in artifacts if art not in existing_artifacts])
            else:
                original_clusters[name] = artifacts
    ...
    @staticmethod
    def _remove_overlapping_clusters(clusters: Clusters, overlap_threshold: float = OVERLAP_THRESHOLD) -> Clusters:
        """
        Removes the clusters with a lot of overlapping artifacts
        :param clusters: The original clusters
        :param overlap_threshold: Represents percent of overlapping artifacts, above which the cluster will be removed
        :return: The filtered clusters
        """
        okay_clusters = {}
        for cluster1_id, artifacts1 in clusters.items():
            for cluster2_id, artifacts2 in clusters.items():
                if cluster1_id == cluster2_id:
                    continue
                if 1 - (len(set(artifacts1).difference(artifacts2)) / len(artifacts1)) < overlap_threshold:
                    okay_clusters[cluster1_id] = artifacts1
        return okay_clusters
