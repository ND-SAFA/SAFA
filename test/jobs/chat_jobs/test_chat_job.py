...
class TestChatJob(BaseJobTest):
    ...
    @mock_anthropic
    def test_run_second_message_no_context(self, ai_manager: TestAIManager):
        """
        Tests that job is completed succesfully.
        """
        second_response = "You're welcome!"
        ai_manager.set_responses([lambda prompt: self.response(prompt, EXPECTED_CONTEXT_IDS, second_response,
                                                               False)])
        job = self._get_job(additional_chats=[MessageMeta(message=Message(content=self.RESPONSE, role=PromptRoles.ASSISTANT)),
                                              MessageMeta(message=Message(content="Thank you!",
                                                                          role=PromptRoles.USER))])
        job.run()
        message_meta: MessageMeta = job.result.body
        self.assertEqual(message_meta.message["content"], second_response)
        self.assertFalse(message_meta.artifact_ids)
