...
class TestSafaExporter(BaseTest):
    ...
    def test_export_generated_links(self):
        ...
        source_artifacts = [Artifact(id="S{i}", content="dummy text", layer_id=source_type) for i in range(3)]
        target_artifacts = [Artifact(id="T{i}", content="dummy text", layer_id=target_type) for i in range(3)]
        ...
        api_definition = ApiDefinition(artifacts=artifacts, layers=layers, links=true_links)
