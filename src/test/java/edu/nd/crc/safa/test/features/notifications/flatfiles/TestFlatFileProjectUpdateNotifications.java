 ...
 */
public class TestFlatFileProjectUpdateNotifications extends AbstractNotificationTest {
    ...
    protected void verifyShareeMessage(List<EntityChangeMessage> messages) {
        ...
        assertionService.verifyArtifactTypeMessage(messages.get(0), parentType);
        assertionService.verifyArtifactTypeMessage(messages.get(1), childType);
        assertionService.verifyTraceMatrixMessage(messages.get(2), List.of(childType, parentType));
        assertionService.verifyProjectEntitiesMessage(messages.get(3), 2, 1);
    }
}
