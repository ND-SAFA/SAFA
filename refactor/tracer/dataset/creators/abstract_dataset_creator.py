...
class AbstractDatasetCreator(ABC):
    ...
    def __init__(self, pre_processor: PreProcessor = None, use_linked_targets_only: bool = False):
        """
    ...
    def _process_artifact_tokens(self, artifact_tokens: Union[List[str], str]) -> Union[List[str], str]:
        """
        Processes the artifact tokens using the given pre-processor
        :param artifact_tokens: artifact content or a list of artifact content strings
        :return: the processed content
        """
        if not isinstance(artifact_tokens, list):
            artifact_tokens = [artifact_tokens]

        if self.pre_processor:
            artifact_tokens = self.pre_processor.run(artifact_content=artifact_tokens)

        if len(artifact_tokens) == 1:
            artifact_tokens = artifact_tokens.pop()
