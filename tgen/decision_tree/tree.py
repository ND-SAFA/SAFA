...
class Tree(abc.ABC):
    ...
    def next_step(self, input_: Any, current_path_taken: Path = None) -> Tuple[PromptBuilder, Path]:
        """
        ...
        if current_node.is_leaf():
            ...
            prompt_builder = current_node.create_prompt_builder(input_)
            path = current_path_taken
        else:
            self._take_step(input_, current_node, current_path_taken)
            prompt_builder, path = self.next_step(input_, current_path_taken)
        return prompt_builder, path

    @staticmethod
    def _take_step(input_: Any, current_node: AbstractNode, path_taken: Path) -> AbstractNode:
        """
        ...
        """
        choice = current_node.make_choice(input_)
        if choice:
