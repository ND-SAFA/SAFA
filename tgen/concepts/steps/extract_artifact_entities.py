...
class EntityExtraction(AbstractPipelineStep):
    ...
    def _run(self, args: ConceptArgs, state: ConceptState) -> None:
        ...
        instructions_prompt = Prompt("Above is an artifact from a software system. "
                                     ...
                                     response_manager=PromptResponseManager(
                                         ...
                                         response_instructions_format=EntityExtraction.get_response_format()))
        response_dict = predict_artifact_prompt(artifact_content, instructions_prompt, args.llm_manager)
    ...
    def get_response_format() -> str:
        return (
            f"Record each acronym like so: \n"
            f"<{EntityExtraction.ENTITY_TAG}>"
            f"<{EntityExtraction.ENTITY_NAME_TAG}>ACRONYM</{EntityExtraction.ENTITY_NAME_TAG}>"
            f"<{EntityExtraction.ENTITY_DESCRIPTION_TAG}>DEFINITION</{EntityExtraction.ENTITY_DESCRIPTION_TAG}>"
            f"</{EntityExtraction.ENTITY_TAG}>"
        )
