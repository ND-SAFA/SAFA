...
def create_hgen_dataset(hgen_args: HGenArgs) -> None:
    """
    ...
    :return: None
    """
    artifact_generations = hgen_args.state.refined_content
    original_dataset_complete = hgen_args.state.original_dataset
    ...
    target_layer_id = _get_target_layer_id(hgen_args, original_dataset_complete)

    new_artifact_df = create_artifact_df_from_generated_artifacts(hgen_args, artifact_generations, target_layer_id)
    save_dataset_checkpoint(PromptDataset(artifact_df=new_artifact_df), export_path, filename="generated_artifacts_only")

    new_layer_df = _create_layer_df_with_generated_artifacts(hgen_args, target_layer_id)
    combined_artifact_df = ArtifactDataFrame.concat(original_artifact_df, new_artifact_df)
    new_trace_df = _create_trace_df_with_generated_artifacts(hgen_args, combined_artifact_df)
    ...
    hgen_args.state.dataset = dataset
