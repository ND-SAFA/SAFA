...
class TestHierarchyGeneratorWithClustering(BaseTest):
    ...
    def assert_generate_trace_links_step(self, expected_names, n_artifacts_per_cluster,
        ...
        for cluster_id, cluster_artifacts in state.seed2artifacts.items():
    ...
    def _reset_cluster_artifacts_for_tracing_test(self, n_artifacts_last_cluster, n_artifacts_per_cluster, state):
        ...
        last_cluster_artifacts = state.seed2artifacts[len(state.generation_predictions) - 1]
        state.seed2artifacts[len(state.generation_predictions) - 1] = last_cluster_artifacts[
    ...
    def _setup_state_post_clustering_step(self, state):
        ...
        state.seed2artifacts = {i: [a[ArtifactKeys.ID] for a in artifacts[i * n:i * n + n]]
                                for i in range(n_clusters)}
        if n_clusters * n < len(artifacts):
            rem = n_clusters * n
            added_clusters = [a[ArtifactKeys.ID] for a in artifacts[rem:] + artifacts[:2]]
            state.seed2artifacts[n_clusters - 1].extend(added_clusters)
            n_artifacts_last_cluster += len(added_clusters)

        manual_clusters = {i: [a_id for a_id in artifact_ids] for i, artifact_ids in state.seed2artifacts.items()}
