...
class LLaMAForSequenceSimilarity(LLaMAPreTrainedModel):
    ...
    def forward(
        ...
        pass1_input_ids, pass2_input_ids = torch.split(input_ids, input_ids.shape[1] // 2, dim=1)
        pass1_attention_mask, pass2_attention_mask = torch.split(attention_mask, attention_mask.shape[1] // 2, dim=1)

        # decoder outputs consists of (dec_features, layer_state, dec_hidden, dec_attn)
        outputs1 = self.model(
            input_ids=pass1_input_ids,
            attention_mask=pass1_attention_mask,
            past_key_values=past_key_values,
            inputs_embeds=inputs_embeds,
            use_cache=use_cache,
            output_attentions=output_attentions,
        ...
        outputs2 = self.model(
            ...
            attention_mask=pass2_attention_mask,
            past_key_values=past_key_values,
            inputs_embeds=inputs_embeds,
            use_cache=use_cache,
            output_attentions=output_attentions,
        ...
        hidden_states = torch.cat((outputs1[0], outputs2[0]), dim=2)
        logits = self.lm_head(hidden_states)

        if pass1_input_ids is not None:
            batch_size, sequence_length = pass1_input_ids.shape[:2]
        else:
            batch_size, sequence_length = inputs_embeds.shape[:2]
        ...
        if self.config.pad_token_id is None:
            ...
            if pass1_input_ids is not None:
                sequence_lengths = (torch.ne(pass1_input_ids, self.config.pad_token_id).sum(-1) - 1).to(logits.device)
            else:
                sequence_lengths = -1
                logger.warning(
                    f"{self.__class__.__name__} will not detect padding tokens in `inputs_embeds`. Results may be "
