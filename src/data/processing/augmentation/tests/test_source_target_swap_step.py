...
class TestSourceTargetSwapStep(BaseTest):
    ...
    def test_run(self):
        ...
        self.assertEquals(len(augmented_data), len(data_entries))
        n_entries = [0, 0]
        for resampled_entry, reference_index in augmented_data:
            if resampled_entry[0] == entry1[1]:
                n_entries[0] += 1
                self.assertEquals(reference_index, 0)
            elif resampled_entry[0] == entry2[1]:
                n_entries[1] += 1
                self.assertEquals(reference_index, 1)
            else:
    ...
    def test_augment(self):
        ...
        self.assertEquals(data_entry[1], swapped_entry[0])
        self.assertEquals(data_entry[0], swapped_entry[1])
