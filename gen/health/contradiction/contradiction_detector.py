...
from gen_common.data.keys.structure_keys import ArtifactKeys, LayerKeys, TraceKeys, TraceRelationshipType
...
class ContradictionsDetector:
    ...
    def detect(self, query_ids: List[str]) -> List[ContradictionsResult]:
        ...
        """
        artifact_df = self.args.dataset.trace_dataset.artifact_df
        assert all([q_id in artifact_df for q_id in query_ids]), f"Queries contain unknown ids: {query_ids}"
        context_traces = []
        id2context = {}
    ...
    def _construct_prompt_builder(self, id2context: Dict[str, List[Artifact]], query_id: str,
        ...
        """
        artifact_df = self.args.dataset.trace_dataset.artifact_df
        query_artifact = artifact_df.get_artifact(query_id)
        ...
        task_prompt: QuestionnairePrompt = CONTRADICTIONS_TASK_PROMPT
