...
class TraceMatrix:
    ...
    def __init__(self, trace_df: TraceDataFrame, predicted_scores: Union[List[float], np.ndarray] = None, link_ids: List[int] = None,
        ...
        """
        self.query_matrix = defaultdict(lambda: Query(links=[], preds=[]))
        self.parent_ids = set()
    ...
    def _fill_trace_matrix(self, trace_df: TraceDataFrame, predicted_scores: List[float] = None, link_ids: List[int] = None) -> None:
        ...
        :return: None
        """
        logger.info("Creating Trace Matrix...")
        ...
        trace_map = trace_df.to_map()
        links = [trace_map[link_id] for i, link_id in enumerate(link_ids)]
        self.add_links(links, predicted_scores)

    def add_links(self, links: List[EnumDict], scores: list[float] = None) -> None:
        """
        Adds a new link to the trace matrix
        :param links: the trace link
        :param scores: The scores associated with each link.
        :return: None
        """
        if not isinstance(links, list):
            raise Exception("Expected list of links.")

        for link, score in tqdm(zip(links, scores), desc="Filling matrix... "):
            parent_id = link[TraceKeys.TARGET]
            self.query_matrix[parent_id].links.append(link)
            self.query_matrix[parent_id].preds.append(score)
