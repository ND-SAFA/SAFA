...
def initialize_datasets(hgen_args: HGenArgs) -> None:
    ...
    original_dataset_complete = hgen_args.dataset_creator_for_sources.create() if hgen_args.dataset_for_sources is None \
        else hgen_args.dataset_for_sources
    save_dataset_checkpoint(original_dataset_complete, export_path, filename="initial_dataset_with_sources")
    source_layer_only_dataset = _create_dataset_with_single_layer(original_dataset_complete.artifact_df,
                                                                  hgen_args.source_layer_id,
                                                                  original_dataset_complete.trace_dataset.trace_df
                                                                  if original_dataset_complete.trace_dataset else None)
    hgen_args.state.source_dataset = source_layer_only_dataset
    hgen_args.state.original_dataset = original_dataset_complete
