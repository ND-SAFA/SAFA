...
class TracingJob(LLMJob):
    ...
    def parse_ranking_response(source, r, source2entries):
        ...
        source_target_ids = list(map(lambda t: t["target"], source_entries))
    ...
    @staticmethod
    def create_ranking_prompts(artifact_map, source2entries):
        sources = []
        prompts = []
        for source, source_predicted_entries in source2entries.items():
            target_ids = list(map(lambda t: t["target"], source_predicted_entries))
