...
class TestState(BaseTest):
    ...
    def test_load_latest(self, anthropic_manager: TestAIManager, project_summary_mock: MagicMock):
        ...
        self.assertSetEqual(set(state.generations2sources.keys()), set(HGenTestConstants.user_stories))
        self.assertListEqual(list(state.generations2sources.values()), [set(files) for files in HGenTestConstants.code_files])
